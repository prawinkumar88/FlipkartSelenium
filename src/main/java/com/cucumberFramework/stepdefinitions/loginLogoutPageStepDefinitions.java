...
public class loginLogoutPageStepDefinitions extends TestBase {
	...
	public void i_should_see_Sign_In_Page() throws Throwable {
		String loginOut=loginPage.userNameVerify.getText();
		System.out.println(loginOut);
		Assert.assertEquals(loginOut, "Enter Email/Mobile number");
	...
	public void i_should_see_user(String user) throws Throwable {
		Thread.sleep(2000);
		String loggedUser=loginPage.loggedUser.getText();
		System.out.println(loggedUser);
		Assert.assertEquals(loggedUser, user);
	}
	
	@Then("^I should see warning as \"([^\\\"]*)\"$")
	public void i_should_see_warning_as(String expectedInvalidUserMsg) throws Throwable {
		Thread.sleep(2000);
		String invalidUserMsg=loginPage.invalidUserMsg.getText();
		System.out.println(invalidUserMsg);
		Assert.assertEquals(invalidUserMsg, expectedInvalidUserMsg);
		
	}
	
	@Then("^I should see warning as enter valid cred \"([^\\\"]*)\"$")
	public void i_should_see_warning_as_enter_valid_cred(String expectedEmptyPasswordMsg) throws Throwable {
		Thread.sleep(2000);
		String emptyPasswordMsg=loginPage.emptyPasswordMsg.getText();
		System.out.println(emptyPasswordMsg);
		Assert.assertEquals(emptyPasswordMsg, expectedEmptyPasswordMsg);
		
	}
	
	@Then("^I should see password warning as \"([^\\\"]*)\"$")
	public void i_should_see_password_warning_as(String expectedInvalidPasswordMsg) throws Throwable {
		Thread.sleep(2000);
		String invalidPasswordMsg=loginPage.invalidPasswordMsg.getText();
		System.out.println(invalidPasswordMsg);
		Assert.assertEquals(invalidPasswordMsg, expectedInvalidPasswordMsg);
